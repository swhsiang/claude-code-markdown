...
The simplest way to enable tool result clearing is to specify only the strategy type, as all other [configuration options](https://platform.claude.com/docs/en/build-with-claude/context-editing#configuration-options-for-tool-result-clearing) will use their default values:
cURL
```
...
Compaction is available in the [Python and TypeScript SDKs](https://platform.claude.com/docs/en/api/client-sdks) when using the [`tool_runner` method](https://platform.claude.com/docs/en/agents-and-tools/tool-use/implement-tool-use#tool-runner-beta).
